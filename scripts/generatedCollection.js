...
document.getElementById('left').style.transform = "translate(0px,0px)";

function fetchCollectionData() {

    //INLOCUIESTE CU INFO DIN LOCALSTORAGE
    document.getElementById('titleText').textContent = localStorage.getItem("title");
    document.title = localStorage.getItem("title") + ' | HeMa';

    const currentDate = new Date();

    const creationDay = currentDate.getDate();
    const creationMonth = currentDate.getMonth() + 1; 
    const creationYear = currentDate.getFullYear();

    document.getElementById('right').innerHTML = 'Uploaded at: ' + creationDay + '/' + creationMonth + '/' + creationYear;
