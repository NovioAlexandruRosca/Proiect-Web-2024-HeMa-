...
const server = http.createServer(async (req, res) => {
    ...
    // ADMIN PAGE -------------------------------
    // USED FOR FETCHING CLIENT DATA FOR ADMIN
    if (req.method === 'GET' && req.url === '/admin/api/clients') {
        headerNotModified = false;
    ...
    // USED FOR DELETING A CLIENT FOR ADMIN
    if (req.method === 'DELETE' && req.url.startsWith('/admin/api/clients/')) {
        headerNotModified = false;
    ...
    if (req.method === 'GET' && req.url === '/admin/api/reports/clients/pdf') {
        headerNotModified = false;
        generateClientsPDFReport(res);
    } 

    if (req.method === 'GET' && req.url === '/admin/api/reports/clients/csv') {
        headerNotModified = false;
        generateClientsCSVReport(res);
    } 

    if (req.method === 'GET' && req.url === '/admin/api/reports/plants/pdf') {
        headerNotModified = false;
        generatePlantsPDFReport(res);
    } 

    if (req.method === 'GET' && req.url === '/admin/api/reports/plants/csv') {
        headerNotModified = false;
...
// GENERATE REPORTS
function generateClientsPDFReport(res) {
    const query = `
  ...
  function generatePlantsPDFReport(res) {
    const query = `
  ...
  function generatePlantsCSVReport(res) {
    const query = `
      SELECT plant_id, owner_id, collection_id, collection_date, hashtags, common_name,
             scientific_name, family, genus, species, place_of_collection, color, number_of_visits
      FROM plants
